...
<template>
  <div :class="styles.container">
    <div :class="styles.topBox">
      <span :class="styles.title">企业客户标签 </span>
      <span :class="styles.desc"
        >企业客户标签由管理员或一级管理规则的负责人配置。配置后，企业成员可以对客户打企业客户标签。
      </span>
    </div>
    <!--    <a-form-->
    <!--      :model="tagParams"-->
    ...
    <div :class="styles.resultBox">
      <div class="right">
        <a-button type="primary" style="margin: 0 10px" @click="handleAddGroup"
          >新增标签组</a-button
        >
            ...
            <template #cell="{ record }">
              <template v-if="record.tags && record.tags.length > 5">
                <a-popover style="width: 300px">
                  <div v-if="record.tags && record.tags.length > 5">
                    ...
                    <a-tag style="margin: 0 8px" color="arcoblue"> ··· </a-tag>
                  </div>
                  <div v-else>
                    <span
                      v-for="(item, index) in record.tags"
              ...
              <template v-else>
                <span
                  ...
                  style="margin: 0 8px"
                >
                  <a-tag v-if="item.name && item.name != ' '" color="arcoblue">
                    {{ item.name }}
                  </a-tag>
                </span>
              </template>

<template>
  <div>
    <a-form
      ...
      @submit="onSubmit"
    >
      <a-form-item label="支付单名称" field="name">
...
<script lang="ts" setup>
  ...
  import { Payment, updatePayment } from '@/api/crm/trade/payment';
  ...
  const formModel = ref({
    id: prop.node?.id,
    orderId: prop.node?.orderId,
    paymentDate: prop.node?.paymentDate,
    paymentType: prop.node?.paymentType,
    paidAmount: prop.node?.paidAmount,
    paymentNumber: prop.node?.paymentNumber,
    referenceNumber: prop.node?.referenceNumber,
    status: prop.node?.status,
    paymentItems: prop.node?.paymentItems ? prop.node?.paymentItems : [],
  } as Payment);

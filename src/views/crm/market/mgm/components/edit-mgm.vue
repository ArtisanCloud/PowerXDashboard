...
<script lang="ts" setup>
  ...
  import { createMGMRule, MGMRule, updateMGMRule } from '@/api/crm/market/mgm';
  import { PropType } from 'vue/dist/vue';

  const prop = defineProps({
    node: {
      type: Object as PropType<MGMRule>,
      default() {
        return {};
      },
    },
  ...
  const formModel = ref({
    id: prop.node?.id,
    name: prop.node?.name,
    commissionRate1: prop.node?.commissionRate1,
    commissionRate2: prop.node?.commissionRate2,
    scene: prop.node?.scene,

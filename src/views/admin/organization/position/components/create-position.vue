...
<script lang="ts" setup>
  ...
  import { createPosition, CreatePositionRequest } from '@/api/position';
  ...
  const formRef = ref();
  const formModel = ref({} as CreatePositionRequest);
  ...
  const onSubmit = async () => {
    ...
    if (!formModel.value.roleCodes?.length) {
      Message.error('请选择权限角色');
      return;
    }
    createPosition(formModel.value).then(() => {

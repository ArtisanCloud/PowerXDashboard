...
export const useOneShotAlert = () => {
  ...
  const color = useState('chat-alert-color', () => 'red')
  const variant = useState('chat-alert-variant', () => 'solid')
  const icon = useState('chat-alert-icon', () => 'i-heroicons-wifi-20-solid')

  const notifyOnce = (t: string, d?: string, c = 'red', v: 'solid' | 'soft' | 'outline' = 'solid') => {
  ...
  const reset = () => { visible.value = false; alertedOnce.value = false }

  return { alertedOnce, visible, title, description, color, variant, icon, notifyOnce, hide, reset }
}

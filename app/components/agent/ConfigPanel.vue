...
const capabilityDict = [
  { key: 'text-generation',  name: t('agent.config.capabilities.textGeneration') },
  { key: 'code-generation',  name: t('agent.config.capabilities.codeGeneration') },
  ...
  { key: 'file-processing',  name: t('agent.config.capabilities.fileProcessing') },
...
watch(() => props.agent, (a) => {
  ...
  if (Array.isArray(caps)) {
    if (caps.length > 0 && typeof caps[0] === 'string') {
      ...
      selectedCaps.value = caps.map((c: any) => c?.id || toKeyFromName(c?.name)).filter(Boolean)
    }
  } else {
    selectedCaps.value = []
  }
...
const systemPromptPresets = [
  { name: t('agent.config.presets.assistant'),   prompt: t('agent.config.presets.assistantPrompt') },
  { name: t('agent.config.presets.coder'),       prompt: t('agent.config.presets.coderPrompt') },
  { name: t('agent.config.presets.translator'),  prompt: t('agent.config.presets.translatorPrompt') },
  { name: t('agent.config.presets.analyst'),     prompt: t('agent.config.presets.analystPrompt') }
]
  ...
  >
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        <!-- 基本信息 -->
        <div class="space-y-4">
          <h3 class="text-lg font-medium text-gray-900">{{ t('agent.config.basicInfo') }}</h3>

          <UFormField :label="t('agent.config.name')" required>
            <UInput v-model="form.name" :placeholder="t('agent.config.namePlaceholder')" />
          </UFormField>

          <UFormField :label="t('agent.config.description')">
            <UTextarea v-model="form.description" :rows="3" :placeholder="t('agent.config.descriptionPlaceholder')" />
          </UFormField>

          <UFormField :label="t('agent.config.avatar')">
            <UInput v-model="form.avatar" :placeholder="t('agent.config.avatarPlaceholder')" />
            <div v-if="form.avatar" class="mt-2">
              <img :src="form.avatar" :alt="form.name" class="w-12 h-12 rounded-full object-cover" @error="form.avatar = ''" />
            </div>
          </UFormField>

          <UFormField :label="t('agent.config.status')">
            <USwitch v-model="form.isActive" :label="form.isActive ? t('agent.config.active') : t('agent.config.inactive')" />
          </UFormField>
        </div>

        <!-- 模型配置 -->
        <div class="space-y-4">
          <h3 class="text-lg font-medium text-gray-900">{{ t('agent.config.modelConfig') }}</h3>

          <UFormField :label="t('agent.config.model')">
            <USelect v-model="form.model" :options="modelOptions" :placeholder="t('agent.config.selectModel')" />
          </UFormField>
          ...
          <UFormField :label="t('agent.config.temperature')">
            <div class="space-y-2">
              <div class="flex items-center gap-3">
                <input type="range" min="0" max="2" step="0.1" v-model.number="form.temperature" class="w-full" />
                <span class="w-10 text-right text-sm text-gray-700">{{ (form.temperature ?? 0.7).toFixed(1) }}</span>
              </div>
          ...
          <UFormField :label="t('agent.config.maxTokens')">
            <UInput v-model.number="form.maxTokens" type="number" :min="1" :max="32000" :placeholder="t('agent.config.maxTokensPlaceholder')" />
          </UFormField>
        </div>
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-medium text-gray-900">{{ t('agent.config.systemPrompt') }}</h3>
            <UDropdownMenu
              :items="[ systemPromptPresets.map(p => ({ label: p.name, click: () => applyPreset(p) })) ]"
            >
          ...
          <UFormField>
            <UTextarea v-model="form.systemPrompt" :rows="6" :placeholder="t('agent.config.systemPromptPlaceholder')" />
          </UFormField>
        </div>

        <!-- 能力配置（用字符串 key 编辑，保存时映射为对象数组） -->
        <div class="space-y-4">
          <h3 class="text-lg font-medium text-gray-900">{{ t('agent.config.capability') }}</h3>
          <div class="grid grid-cols-2 gap-3">
            <div v-for="cap in capabilityDict" :key="cap.key" class="flex items-center space-x-2">
              <UCheckbox :checked="hasCapability(cap.key)" @change="toggleCapability(cap.key)" />
              <label class="text-sm text-gray-700 cursor-pointer">{{ cap.name }}</label>
            </div>
          </div>
        </div>
        ...
        <!-- 高级设置 -->
        <div class="space-y-4">
          <h3 class="text-lg font-medium text-gray-900">{{ t('agent.config.advancedSettings') }}</h3>

          <UFormField :label="t('agent.config.contextWindow')">
            <UInput v-model.number="form.contextWindow" type="number" :placeholder="t('agent.config.contextWindowPlaceholder')" />
          </UFormField>
          ...
          <UFormField :label="t('agent.config.responseFormat')">
            <USelect
              ...
              :options="[
                ...
                { label: t('agent.config.formats.markdown'), value: 'markdown' },
          ...
          <UFormField :label="t('agent.config.streaming')">
            <USwitch v-model="form.streaming" :label="form.streaming ? t('agent.config.streamingEnabled') : t('agent.config.streamingDisabled')" />
          </UFormField>
        </div>
      </div>

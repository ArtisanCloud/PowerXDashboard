      ...
      <template #header>
        <div class="workflow-header">
          ...
          <div class="workflow-actions">
            <UButton icon="i-heroicons-document-plus" color="primary" @click="createNewWorkflow">
              新建工作流
            </UButton>
            <UButton icon="i-heroicons-folder-open" color="neutral" variant="ghost" @click="openWorkflow">
              打开工作流
            </UButton>
          </div>
        </div>
    ...
    <UModal v-model="showNewWorkflowModal">
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <UFormField label="描述">
            <UTextarea v-model="newWorkflow.description" placeholder="输入工作流描述（可选）" />
          </UFormField>
        </div>
        
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton color="neutral" variant="ghost" @click="showNewWorkflowModal = false">
              取消
            </UButton>
            <UButton color="primary" :disabled="!newWorkflow.name" @click="confirmCreateWorkflow">
              创建
            </UButton>
          </div>
        </template>
      ...
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <UTable v-else :columns="workflowColumns" :rows="workflowList">
            <template #actions-data="{ row }">
              <UButton color="primary" size="sm" @click="loadSelectedWorkflow(row.id)">
                打开
              </UButton>
            </template>
          </UTable>
        ...
        <template #footer>
          <div class="flex justify-end">
            <UButton color="neutral" variant="ghost" @click="showOpenWorkflowModal = false">
              取消
            </UButton>
          </div>
        </template>

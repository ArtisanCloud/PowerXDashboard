...
## 摘要
- PowerX Admin 以 Nuxt 4 构建为 SPA，统一从 `runtimeConfig.public` 读取 API/WebSocket 入口并启用暗色优先的 UI 外观（`nuxt.config.ts:7-52`）。  
...
## 项目定位
PowerX Admin 是 PowerX 体系的运营与集成控制台，提供多租户 Agent 管理、插件生态运营与工作流配置等能力。应用注册为 Nuxt 4 单页模式（`nuxt.config.ts:7-74`）并将品牌名称/描述写入 `<head>` 元信息及开机过渡动画（`app/app.vue:4-25`）。  

...
> 提供将 PowerX Admin 部署在 Docker 环境（单机或 K8s）的推荐实践，涵盖镜像构建、环境注入、日志/健康检查与弹性扩缩。
...
- 运行容器时注入 `.env`：  
  ```bash
  docker run --env-file .env.production -p 3000:3000 powerx-admin
  ```
  ...
  spec:
    ...
    template:
      spec:
        containers:
          - name: app
            image: registry.example.com/powerx-admin:latest
...
- 在 `CI_CD_Pipeline_for_Nuxt.md` 构建后执行：
  ```bash
  docker build -t registry.example.com/powerx-admin:${GIT_SHA} .
  docker push registry.example.com/powerx-admin:${GIT_SHA}
  ```
...
- 保留上一版镜像 Tag：`registry.example.com/powerx-admin:rollback`.  
- Kubernetes：`kubectl rollout undo deployment/powerx-admin`.  

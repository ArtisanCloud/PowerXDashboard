...
> 总结 PowerX Admin 的部署形态：Vercel/静态托管、Node SSR、Docker + Nginx 反向代理，并附上 WebSocket/SSE 所需的配置要点。
...
3. 推荐配合 PM2：
   ```bash
   pm2 start .output/server/index.mjs --name powerx-admin
   pm2 save
   ```
...
运行：
```bash
docker build -t powerx-admin .
docker run -p 3000:3000 --env-file .env.production powerx-admin
```
